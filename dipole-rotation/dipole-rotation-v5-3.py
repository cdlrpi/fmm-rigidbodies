...
results = np.array([[example(alpha,0,0,d,p)                      for d in dist] for alpha in theta])
...
error_multipole = np.abs((VB - PhiB))/VB_rms*100
...
np.savetxt('error_exact.mat', error_exact, fmt='%3.16f')  
...
# # CS = plt.contourf(R, theta*180/np.pi, error_exact)
...
# # plt.rc('font', family='serif')
# # levels = np.linspace(0,20,10)
# # CS = plt.contour(R, theta*180/np.pi, error_multipole, levels)
# # CS = plt.contour(R, theta*180/np.pi, bound, levels)
# # CB = plt.colorbar(CS, extend='both')
# # CB.set_label(r'$\%$ Error')
